...
export default function PointsFortsPage() {
  return (
    <>
      <SeoHead
        title="Points Forts"
        description="Les points forts Go Black Motors: ponctualité, discrétion, confort, réservation rapide et service premium."
      />
      <SiteLayout>
        <PageHero
            ...
            {pointsFortsContent.items.map((item, index) => (
              <FadeIn key={item.number} delay={index * 0.08}>
                <article className="rounded-[2rem] bg-neutral-100 p-8 md:p-10 min-h-[18rem] flex flex-col justify-between">
                  <span className="text-5xl md:text-6xl font-black text-amber-600">
                    {item.number}
                  </span>
                  <div>
